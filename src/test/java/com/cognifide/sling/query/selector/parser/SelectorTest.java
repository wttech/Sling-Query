...
public class SelectorTest {
	...
	public void parseMultiSegments() {
		List<SelectorSegment<Resource>> segments = getSegments("cq:Page cq:Page");
	...
	private static List<SelectorSegment<Resource>> getSegments(String selector) {
		return SelectorParser.parse(selector, SearchStrategy.DFS, new ResourceTreeProvider()).getSegments();
	}
	...
	private static List<SelectorSegment<?>> getSegments(String... segments) {
		...
		for (int i = 1; i < segments.length; i += 2) {
			...
			SelectorSegment<?> segment = new SelectorSegment<Resource>(parsed.getResourceType(), null,
					parsed.getAttributes(), parsed.getFunctions(), operator, SearchStrategy.DFS,
					new ResourceTreeProvider());
			list.add(segment);
		}
		return list;
	}
